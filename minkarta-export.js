            ...
            if (o.typ === 'verkansomrade') {
                ...
                // laser fran vanster till hoger.
                {
                    ...
                    [rotation - halfSpread, rotation + halfSpread].forEach(bearing => {
                        const br = bearing * Math.PI / 180;
                        const dLat = (halfRange * Math.cos(br)) / R * 180 / Math.PI;
                        const dLng = (halfRange * Math.sin(br)) / (R * cosLat) * 180 / Math.PI;
                        const m = project(o.lat + dLat, o.lng + dLng);
                        let css = (((bearing % 360) + 360) % 360) - 90;
                        if (css > 90) css -= 180;
                        if (css < -90) css += 180;
                        ctx.save();
